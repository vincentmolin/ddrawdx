...
def flower(c):
    for s in jnp.arange(0.0, 0.8, 0.05):
        ...
        c = drx.fill_rect(
            c, -0.9 + s, -0.9 + s, 0.9 - s, 0.9 - s, reddish * (1 - s), 400 * (0.88 - s)
        )
    return c
...
ps = jnp.array([[0.1, 0.1], [0.1, 0.5], [0.5, 0.1]])
...
c = drx.canvas(500, 500, "RGB")
c, _ = drx.origin(c)
drx.show(c)
for i in range(65):
    c = drx.fill_rect(c, -0.1, -0.1, 0.1, 0.5, red * i / 65)
    c, r = drx.rotate(c, 0.1)
drx.show(c)
